...
- The 'Role - DefaultScoped' and the 'DefaultScope' permissions will be updated in DataStorage when a different entitlement for the same role is granted and the Mapping file is modified. This ensures that the desired scope from the mapping is directly granted by HelloID. If this is not done, you must first update the 'Role - DefaultScoped' or 'DefaultScope' before granting a different entitlement for the same role. Currently, there is no processing order between entitlements. Otherwise, you must run 'Force Update Permission' twice after each CSV mapping change.
