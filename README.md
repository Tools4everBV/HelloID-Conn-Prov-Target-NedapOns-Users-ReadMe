...
#### MappingFiles
  The mapping files are used for both the role assignments and the Default scope in the permission scripts. Supposing the mapping applied between HR en Nedap is the same. Although for the Defaultscope extra columns are added AllEmployees and AllClients. These columns are ignored in the role assignments. The 'All' options for the role assignments are managed with separate entitlements.

#### Business Rules Validation Check

In certain situations, it may occur that an employment with the reference number 1000467-1 has an account entitlement, while another employment with the reference number 1000467-2 has been granted permissions for the Defaultscope or Provisioning Role. This lead to a mismatch between the account reference and the contracts in scope. This mismatch is a result of an incorrect configuration of the Business Rules.
The connector checks for this mismatch and will generate a "warning" audit log, but the connector will still complete successfully without processing the permission. And if the Account Reference was removed the permissions will be removed from Nedap.
It is important to ensure that by granting permissions to specific employment, they also have an associated account entitlement.

#### Processing Multiple Accounts

Because of support of multiple accounts for Nedap, you have the situation that the Update task will remove an account. This circumstance encounters the following problem: By default, the process order of a revoke trigger is first revoke the permission, and after revoke the account entitlement. So the permissions are revoked before the account entitlement is out of scope. *Documentation shown [Here](https://docs.helloid.com/hc/en-us/articles/360012715240-The-User-Account-Lifecycle).*
In our situation, this works differently, because the update task removes the account. The process order will be in reverse, first the account revocation and then the permission. The difference in process order leads that the removed account Reference does not appear in the permission task. So the permissions cannot be removed. The permission script performs sort of a cleanup to revoke the permissions of the earlier removed accounts the next time it runs. Although this is not a straight process and will be triggered the next time the specific permission updates, or manually force update permission.
